...
def plot_adaptive_strength(output_path: Path = config.RESULTS_DIR / "adaptive_strength.png") -> Path:
    runs = load_seed_runs("adaptive")
    if not runs:
        raise FileNotFoundError("No adaptive runs found under results/adaptive/.")

    matrix = column_matrix(runs, "mutation_strength")
    generations = np.arange(1, matrix.shape[1] + 1)
    mean = matrix.mean(axis=0)
    std = matrix.std(axis=0)
    ...
    ax.axhline(config.ADAPTIVE_INITIAL_STRENGTH, color="grey", linestyle="--", linewidth=1, label="initial")
    ax.set_xlabel("Generation")
    ax.set_ylabel("Mutation strength (macro-op share)")
    ax.set_title("Adaptive mutation strength over generations (mean ± std)")
    ax.legend()
    fig.tight_layout()
    fig.savefig(output_path, dpi=150)
    plt.close(fig)
...
def main() -> None:
    ...
    print(f"Saved: {plot_adaptive_strength()}")   # was: plot_adaptive_probability
